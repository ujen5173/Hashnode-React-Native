...
import { ArrowLeft, Calendar, ChevronRight, Facebook, Flag, Github, Globe, Instagram, Layers, Linkedin, MapPin, MoreVertical, Twitter, X, Youtube } from 'lucide-react-native';
  ...
  return (
    <BottomSheetModalProvider>
      <ScrollView style={tw`flex-1 p-4 bg-slate-100 dark:bg-slate-900`}>
        ...
        <View style={tw`border border-slate-300 dark:border-slate-600 bg-slate-200 dark:bg-slate-800 p-4 rounded-lg my-4`}>
          <View style={tw`flex-row items-center gap-2 mb-3`}>
            <MapPin size={18} style={tw`text-slate-700 dark:text-slate-400`} />
            <Text style={tw`text-base text-slate-700 dark:text-slate-400`}>{data?.data?.location}</Text>
          </View>
          <View style={tw`flex-row items-center gap-2`}>
            <Calendar size={18} style={tw`text-slate-700 dark:text-slate-400`} />
            <Text style={tw`text-base text-slate-700 dark:text-slate-400`}>Member since {data?.data?.createdAt && format(new Date(data?.data?.createdAt!), "MM, yyyy")}</Text>
          </View>
        </View>

        <View style={tw`my-4`}>
          {/* socials */}
          <Text style={tw`text-slate-800 dark:text-slate-200 text-xl mb-4 font-bold`}>My socials</Text>
          <View style={tw`flex-row gap-4 justify-center flex-wrap`}>
            {
              data?.data?.social && Object.entries(data?.data?.social).map(([key, value]) => {
                if (!value) return null;
                return (
                  <TouchableOpacity activeOpacity={.9} onPress={() => {
                    Linking.openURL(value)
                  }} key={key} style={tw`rounded-full p-4 border border-slate-300 dark:border-slate-600`}>
                    {socials[key]}
                  </TouchableOpacity>
                )
              }
          ...
          {/* blogging area */}
          <View style={tw`my-4`}>
            <Text style={tw`text-slate-800 dark:text-slate-200 text-xl mb-4 font-bold`}>Writes at</Text>

            <View style={tw`flex-row gap-2 border border-slate-300 dark:border-slate-600 bg-slate-200 dark:bg-slate-800 p-3 rounded-lg`}>
              <Image
              ...
              <View style={tw`flex-1 items-start`}>
                <Text style={tw`text-slate-800 dark:text-slate-100 mb-1 text-base`}>Hack, Stack and Roll</Text>
                <View style={tw`flex-row gap-2`}>
                  <Text style={tw`text-slate-700 flex-1 dark:text-slate-400`}>hashnode-t3.vercel.app</Text>
                  <ChevronRight size={18} style={tw`text-slate-500 dark:text-slate-400`} />
                </View>
              </View>
            </View>
          </View>
          ...
          {/* about me */}
          <View style={tw`my-4`}>
            <Text style={tw`text-lg font-bold text-slate-800 dark:text-slate-100 mb-4`}>About me</Text>
          ...
          {/* tech stack */}
          <View style={tw`my-4`}>
            <Text style={tw`text-lg font-bold text-slate-800 dark:text-slate-100 mb-4`}>My tech stack</Text>
            <View style={tw`flex-row gap-2 flex-wrap`}>
              {
                data?.data?.skills.map(lang => (
                  <TouchableOpacity key={lang} style={tw`bg-slate-200 dark:bg-slate-800 py-2 px-4 rounded-full`}>
                    <Text
...
const BottomSheet = ({ handlePresentModalClose }: { handlePresentModalClose: () => void }) => {
